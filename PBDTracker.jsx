  ...
  return (
    <>
      <ModalShell title={`Lampiran - ${studentName}`} onClose={onClose} wide>
        ...
        <button onClick={() => setUploadMode("add")} className="w-full flex items-center justify-center gap-2 text-sm font-semibold rounded-xl py-2.5 border border-dashed" style={{ borderColor: "#D9A9D3", color: "#8B5A9E" }}>
...
/* ===========================================================
   LAPORAN CETAK / PDF
=========================================================== */
function PrintReport({ subject, classObj, students, records }) {
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr>
            ...
            {tajukList.map((tj) => <th key={tj.id} className="border border-black px-2 py-1.5 text-left">{tj.tajuk}</th>)}
          </tr>
        </thead>
        <tbody>
          {students.map((student, idx) => (
            <tr key={student.id}>
              ...
              {tajukList.map((tj) => {
                const tp = records[recKey(subject.id, classObj.id, tj.id, student.id)]?.tp;
                return <td key={tj.id} className="border border-black px-2 py-1.5">{tp ? `TP${tp}` : "-"}</td>;
              })}
            </tr>
          ))}
        </tbody>
...
export default function App() {
  ...
  const [activeTajukId, setActiveTajukId] = useState(DEFAULT_SUBJECTS[0].tajuk[0].id);
  ...
  if (isPrinting && activeSubject) {
    return <PrintReport subject={activeSubject} classObj={selectedClass} students={students} records={records} />;
  }
    ...
    <div className="min-h-screen pb-24 sm:pb-8" style={{ background: UI.bgApp, fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      ...
      {manageStudentsOpen && (
        <ManageItemsModal title={`Urus Murid - ${selectedClass?.name}`} placeholder="Nama murid baru"
          items={students.map((s) => ({ id: s.id, label: s.name }))} onAdd={addStudent} onEdit={editStudent} onDelete={deleteStudent} onClose={() => setManageStudentsOpen(false)} />
      )}
      {manageSubjectsOpen && (
